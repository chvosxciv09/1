...
import { Project, ProjectStatus, DesignPhase, LogEntry, WhiteboardNote, Member } from './types';
import { ChevronRight, ArrowLeft, LayoutList, Users, PieChart, BarChart3, Clock, Lightbulb } from 'lucide-react';
...
const MOCK_PROJECTS: Project[] = [
  {
    ...
    dueDate: '2023-12-15',
    team: [
        { id: 'u1', name: 'Alex', role: '设计主管', avatar: 'https://picsum.photos/100/100?random=1' },
        { id: 'u2', name: 'Sarah', role: '建模师', avatar: 'https://picsum.photos/100/100?random=2' }
    ],
    feedbackLogs: [],
    whiteboardNotes: [
  ...
  },
  {
    ...
    team: [
         { id: 'u1', name: 'Alex', role: '设计主管', avatar: 'https://picsum.photos/100/100?random=1' },
         { id: 'u3', name: 'Mike', role: '结构工', avatar: 'https://picsum.photos/100/100?random=3' },
         { id: 'u4', name: 'Jen', role: 'CMF设计', avatar: 'https://picsum.photos/100/100?random=4' }
    ],
    feedbackLogs: [],
    whiteboardNotes: []
  },
  ...
  },
  {
    ...
    team: [
        { id: 'u1', name: 'Alex', role: '设计主管', avatar: 'https://picsum.photos/100/100?random=1' },
        { id: 'u5', name: 'David', role: '设计师', avatar: 'https://picsum.photos/100/100?random=5' }
    ],
    feedbackLogs: [],
    whiteboardNotes: []
  }
...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'analysis' | 'brainstorm' | 'team'>('analysis');
  ...
  // Simple Phase Timeline Visualization Component
  const PhaseVisualization = () => {
    const phases = Object.values(DesignPhase);
    ...
    return (
      <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6 mb-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="text-xs text-slate-400">实时进度概览</span>
        </div>
        ...
        <div className="space-y-6">
          {projects.map(project => (
            <div key={project.id} className="relative">
              <div className="flex justify-between text-xs mb-2">
                <span className="font-medium text-slate-200 w-32 truncate">{project.name}</span>
                <span className="text-slate-400">{project.currentPhase} ({project.progress}%)</span>
              </div>
              <div className="h-3 bg-slate-700/50 rounded-full overflow-hidden flex relative">
                {/* Background Grid Lines for Phases */}
                <div className="absolute inset-0 flex">
                   {[1,2,3,4,5,6].map(i => (
                     <div key={i} className="flex-1 border-r border-slate-600/30 last:border-0"></div>
                   ))}
                </div>
                ...
                <div 
                  className={`h-full rounded-full transition-all duration-700 ${
                    project.status === ProjectStatus.REVIEW ? 'bg-amber-500' :
                    project.status === ProjectStatus.COMPLETED ? 'bg-emerald-500' :
                    'bg-indigo-500'
                  }`}
          ...
          {/* Phase Labels Axis */}
          <div className="flex text-[10px] text-slate-500 pt-2 border-t border-slate-700/30 mt-4">
        ...
        return (
            <div className="flex flex-col h-full animate-fadeIn">
                 ...
                 <div className="flex-1 min-h-0">
                    ...
                    {activeTab === 'overview' && (
                         <div className="bg-slate-900/50 rounded-xl border border-slate-700/50 p-8 text-center text-slate-500 h-full">
                            <LayoutList size={48} className="mx-auto mb-4 opacity-50" />
                            <h3 className="text-lg font-medium text-slate-300">项目详细信息</h3>
                            <p className="mb-6">这里将展示甘特图、详细的时间线节点以及具体的项目规格书。</p>
                            <div className="max-w-md mx-auto bg-slate-800 p-4 rounded-lg text-left">
                                <h4 className="text-slate-300 font-bold mb-2">项目描述</h4>
    ...
    return (
        <div className="h-full overflow-y-auto pr-2">
            <header className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-white mb-2">欢迎回来, {CURRENT_USER.name}</h1>
                    <p className="text-slate-400">您目前有 {projects.length} 个活跃项目，其中 1 个需要立即关注。</p>
                </div>
                <div className="flex items-center gap-4">
                     <div className="text-right">
                        <p className="text-sm font-bold text-white">{CURRENT_USER.name} Designer</p>
                        <p className="text-xs text-slate-500">{CURRENT_USER.role}</p>
                     </div>
                     <img src={CURRENT_USER.avatar} className="w-10 h-10 rounded-full border border-slate-600" alt="Profile" />
                </div>
            </header>
